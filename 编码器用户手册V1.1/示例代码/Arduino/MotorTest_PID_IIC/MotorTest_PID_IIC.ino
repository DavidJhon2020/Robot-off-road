...
int pot = A2;
#define Encoder1A  7     // 电机1A相 D4
#define Encoder1B  6     // 电机1B相 D2 
...
void loop() {
  Setpoint1 = analogRead(pot) * 4;

  if ( millis() - last > 100)
  {
    double rpm1 = count_rpm1();
    //    Input1 = map(rpm1, -200, 200 , -4096, 4096);
    if (Input1 > 0) {
      Input1 = map(rpm1, 0, 56 , 0, 4096);
    } else {
      Input1 = map(rpm1, 0, -56 , 0, -4096);
    }
  }
  ...
  moveMotor(0, 0, Output1, 0); // 电机M1,M2,M3,M4 运动
}
